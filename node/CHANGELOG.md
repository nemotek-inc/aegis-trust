...
- **Silence is the failure mode this exists for.** A boundary that has stopped
  ...
  last time, again" separates it from a healthy one. `AttestState` / `loadState` / `saveState` holds the
  ...
  start, and `requirePrevious` makes a missing baseline a failure. The stored baseline
  ...
  back. `reportArrivedCheck` raises the report that never arrived, on the SDK's own clock.
...
- Honesty boundary, restated from contract §6: the attestation does not prove
  **decryptability** (bodies are AEAD-sealed against the sealing host) and does
  ...
  `judged: 1` and is otherwise clean, so `AttestExpectation.capsuleIds` is where the expected set
...
- CLI `aegis attest-verify`, exit 0 clean / 1 abnormal / **2 no report could be
  ...
  unauthenticable one.
- Ed25519 verification is `src/ed25519.ts`, OpenSSL via `node:crypto`. The
  Python SDK cannot do the same (its LITE install is dependency-free by
  contract), so it implements RFC 8032 itself — two implementations of one
  primitive stop agreeing silently, so both read
  ...
  rejections including the unreduced scalar `S + L`). Verification only; there
  is no signing code in either SDK.
